...
$errors = [];
$success = "";
$surname = $name = $otchestvo = $group_id = $student_card = $phone = $email = $username = $password = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO user (user_type_id, surname, name, otchestvo, group_id, student_card, phone, email, username, password) 
                VALUES (1, '$surname', '$name', '$otchestvo', '$group_id', '$student_card', '$phone', '$email', '$username', '$hashed_password')";
        
        if (mysqli_query($db, $sql)) {
            $success = "Регистрация прошла успешно! Теперь вы можете войти в систему.";
            $surname = $name = $otchestvo = $student_card = $phone = $email = $username = $password = '';
            $group_id = 0;
        } else {
